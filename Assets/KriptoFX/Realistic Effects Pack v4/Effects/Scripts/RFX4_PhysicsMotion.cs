...
public class RFX4_PhysicsMotion : MonoBehaviour
{
    ...
    public float collideAfter;
    float collideTime;
    void OnEnable ()
    {
        collideTime = Time.time + collideAfter;
        Debug.Log("Set Time" + collideTime);
    ...
    {
        if (Time.time < collideTime) { Debug.Log("Not Time Yet."); return; }
        ...
        foreach (ContactPoint contact in collision.contacts)
        {
            ...
            {
                isCollided = true;
                Debug.Log("Collide: "+contact.thisCollider.name);
