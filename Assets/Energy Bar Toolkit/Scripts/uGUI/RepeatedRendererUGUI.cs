...
public class RepeatedRendererUGUI : EnergyBarUGUIBase {
    ...
    public float repeatRotation = 0;
    public bool repeatKeepOrientation = false;
    public bool repeatKeepPosition = false;
    ...
    private bool RebuildNeeded() {
        ...
        ch = MadHashCode.Add(ch, repeatRotation);
        ch = MadHashCode.Add(ch, repeatKeepOrientation);
        ch = MadHashCode.Add(ch, repeatKeepPosition);
    ...
    private void BuildIconsAndSlots() {
        ...
        for (int i = 0; i < repeatCount; ++i) {
            ...
            if (spriteSlot.sprite != null) {
                ...
                slot.transform.localPosition = ComputePosition(i);
                if (repeatKeepOrientation) {
                    slot.transform.localRotation = Quaternion.identity;
                } else {
                    slot.transform.localRotation = Quaternion.Euler(0, 0, repeatRotation * i);
                }
            ...
            if (spriteIcon.sprite != null) {
                ...
                icon.transform.localPosition = ComputePosition(i);

                if (repeatKeepOrientation) {
                    icon.transform.localRotation = Quaternion.identity;
                } else {
                    icon.transform.localRotation = Quaternion.Euler(0, 0, repeatRotation * i);
                }
        ...
        for (int i = 0; i < slotImages.Count; i++) {
            var slotImage = slotImages[i];
            slotImage.rectTransform.localPosition = ComputePosition(i) + (startPos + offset);
        }

        for (int i = 0; i < iconImages.Count; i++) {
            var iconImage = iconImages[i];
            iconImage.rectTransform.localPosition = ComputePosition(i) + (startPos + offset);
    ...
    private Vector2 ComputePosition(int count) {
        if (repeatKeepPosition || Mathf.Approximately(0, repeatRotation)) {
            return repeatPositionDelta * count;
        } else {
            Vector2 position = Vector2.zero;
            float rotation = Vector2.Angle(Vector2.right, repeatPositionDelta.normalized);

            for (int i = 0; i < count; ++i) {
                rotation += repeatRotation;

                float y = Mathf.Sin(Mathf.Deg2Rad * rotation);
                float x = Mathf.Cos(Mathf.Deg2Rad * rotation);

                position += new Vector2(x * repeatPositionDelta.magnitude, y * repeatPositionDelta.magnitude);
            }
