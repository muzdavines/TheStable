...
public class StageTester : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (heroes.Count == 0) {
            for (int i = 0; i < 4; i++) {
                Character c = new Character(new CharacterSO()) { myName = Names.Warrior[Random.Range(0, Names.Warrior.Length)], age = 18, health = 3, modelName = "CharRogue", activeForNextMission = true };
